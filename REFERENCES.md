...
- `faer 0.24.4` — later sparse matrix storage, sparse QR/least-squares and optional Cholesky fast paths. Do not add it to the active solve path before M7.  
...
- `petgraph 0.8.3` — variable/residual incidence graph, connected components and later matching/decomposition.  
  https://docs.rs/petgraph/latest/petgraph/
- `num-dual 0.14.1` — candidate local forward-mode automatic differentiation. Evaluate only after the residual-block API works with analytic derivatives.  
...
- `criterion 0.8` — M7+ benchmarks; performance is not a substitute for validity.
- central finite differences — mandatory in-house Jacobian oracle even if `num-dual` is adopted.
...
- `solverang 0.1` — feature claims are directly relevant, but it was too new and had too little public provenance/adoption at scaffold time. Reassess later; do not make it foundational now.
- `inari 2` — optional interval arithmetic for future pruning/certification, not MVP solving.
...
- Baraff constrained dynamics notes for the later force/dynamics boundary: https://www.cs.cmu.edu/~baraff/sigcourse/notesf.pdf

### Constraint decomposition/research

- witness-configuration decomposition survey/paper: https://arxiv.org/abs/1811.11472

Use structural mobility counts as heuristics only. Current-configuration Jacobian rank remains the local numerical truth.

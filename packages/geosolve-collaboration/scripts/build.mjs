...
await build({ entryPoints: [resolve(root, "src/index.ts"), resolve(root, "src/host.ts")], outdir: resolve(root, "dist"), bundle: true, format: "esm", platform: "neutral", target: "es2022" });
...
const program = ts.createProgram([source, resolve(root, "src/host.ts")], options);

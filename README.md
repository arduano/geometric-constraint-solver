...
3. `PLAN.md` — active two-deliverable roadmap and ordered milestones M8-M55.
...
- `geosolve-demo-web` — separate desktop WASM/SVG diagnostic consumer without equations or authoritative document semantics; M39-M51 plan its CAD-like sketch-workbench rewrite.
...
and application undo. Four consolidated human UAT gates occur at M40, M45, M52 and
M54; all objective behavior is automated first. The desktop demo has no future mobile
support requirement.
...
The complete release gate is `scripts/release-gate.sh`. Development runs from a
dirty tree use `GEOSOLVE_ALLOW_DIRTY=1 scripts/release-gate.sh`; release evidence

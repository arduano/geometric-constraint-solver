...
const propertyKey = (path) => JSON.stringify(path);
const pointProperty = (address) => JSON.stringify(["point", address]);
...
export async function openCollaborationRuntime(folder, {
  initialize = false, invitations, limits, storageOptions, domainOptions, allowedOrigins = [], workbenchScenes = false, documentId = randomUUID(),
} = {}) {
  ...
  try {
    if (initialize) {
      ...
      semantic = await createTrustedSemanticHost({ configuration: { documentEpoch, serverEpoch, objects: accepted.inventory.objects.map(({ object, dependencies }) => ({ object, dependencies })) } });
      ...
      initial = { configuration: { documentId, documentEpoch, initialInput: accepted.acceptedInput }, checkpoints: {
        source: Buffer.from(source.checkpoint()), model: encode(accepted.model), targets: Buffer.from(checkpoint.targetsJson), history: Buffer.from(checkpoint.historyJson),
      } };
    }
    host = await openDurableCollaborationHost(folder, {
      ...
      rebuild: async ({ configuration, checkpoints, acceptedRevision, acceptedInput }) => {
        ...
        try {
          ...
          const rebuilt = await domain({ kind: "rebuild", folder, files: snapshot.accepted.files, model: JSON.parse(checkpoints.model) });
          ...
          for (const item of rebuilt.inventory.objects) if (!restoredSemantic.current(item.object)) throw Error("Accepted model has a missing semantic target lifetime");
    ...
    const generationSnapshot = () => Object.fromEntries(accepted.inventory.objects.map(({ object }) => [object, semantic.current(object)]));
    ...
    function receiveText(connection, body) {
      ...
      const stage = source.stageTextChanges(body.changes.map((value) => Uint8Array.from(value)), actor);
    ...
    async function execute(prepared, attachments) {
      ...
      try {
        if (prepared.command.kind === "semantic") {
          ...
          const changes = inverse.changes.map((change) => {
            semantic.authenticate(change.address.target);
            const object = accepted.inventory.objects.find((item) => item.object === change.address.target.object);
            if (!object) throw Error("Inverse target no longer has an accepted declaration");
            return { change, declaration: object.declaration, path: JSON.parse(change.address.property) };
          });
          if (changes.some(({ path }) => path[0] === "point" && typeof path[1] === "object")) {
            if (changes.some(({ path }) => path[0] !== "point" || typeof path[1] !== "object")) throw Error("Mixed native point and source inverse is unavailable");
            result = await domain({ kind: "point_properties", folder, files: basis.files, model: accepted.model,
              writes: changes.map(({ change, path }) => ({ address: path[1], expected: change.before, value: change.after })) });
          } else result = await domain({ kind: "values", folder, files: basis.files, model: accepted.model,
            writes: changes.map(({ declaration, path, change }) => ({ declaration, path, value: change.after })) });
        ...
        return () => {
          ...
          try {
            ...
            return { completion: { status: "accepted", acceptedInput: result.acceptedInput, summary: prepared.command.kind === "apply" ? "Applied captured shared draft" : `${prepared.command.kind} accepted` },
              checkpoints: { source: Buffer.from(sourceStage.checkpointJson), model: encode(result.model), targets: Buffer.from(semanticStage.targetsJson), history: Buffer.from(semanticStage.historyJson) },
    ...
    function stageSemanticResult(result, prepared, basisRevision, revision) {
      ...
      const changes = (result.valueChanges ?? []).filter((entry) => !createdNames.has(objectName(result.model.entry, entry.write.declaration))).map(({ write, before }) => ({
        address: { target: semantic.current(objectName(result.model.entry, write.declaration)), property: propertyKey(write.path) }, before, after: write.value,
      }));
      for (const change of result.pointChanges ?? []) changes.push({
        address: { target: semantic.current(change.object), property: pointProperty(change.address) }, before: change.before, after: change.after,
      });
      ...
      return semantic.stageValidatedTransaction({ basisRevision, revision,
        ...
        dependencies: [...newObjects.values()].filter((item) => !createdNames.has(item.object) && !same(item.dependencies, oldObjects.get(item.object).dependencies))
          .map((item) => ({ target: reference(item.object), dependencies: item.dependencies.map(reference) })),
        ...(changes.length ? { record: { operation, changes } } : {}),
      });
    }
    ...
    transport = createCollaborationHttpServer({ host, invitations, execute, captureApply, receiveText, documentSnapshot,
      textDelta: (_connection, revision) => source.textChangesSince(revision),
      scene: () => encode(acceptedScene ?? accepted.result), limits: limits?.http, allowedOrigins,
    });
    let closing;
    return { host, source: () => source, semantic: () => semantic, transport,
      async listen(port = 0, hostname = "127.0.0.1") { await new Promise((resolve, reject) => { transport.server.once("error", reject); transport.server.listen(port, hostname, resolve); }); transport.kick(); return transport.server.address(); },
      close() {

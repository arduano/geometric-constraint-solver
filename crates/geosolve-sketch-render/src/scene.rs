...
pub use geosolve_constraint_editor::{
...
impl RetainedCameraTransform {
    pub fn between(exact: CanvasCamera, desired: CanvasCamera) -> Option<Self> {
        let exact = exact.viewport();
        let desired = desired.viewport();
...
mod tests {
    ...
    fn populated_static_composition_contains_only_durable_paint() {
        ...
        for point in [
            ...
            assert!(
                point.x >= FIT_MARGIN_PIXELS - 1.0e-7
                    && point.x <= viewport.screen_size[0] - FIT_MARGIN_PIXELS + 1.0e-7
            );
            assert!(
                point.y >= FIT_MARGIN_PIXELS - 1.0e-7
                    && point.y <= viewport.screen_size[1] - FIT_MARGIN_PIXELS + 1.0e-7
            );
        }
    }

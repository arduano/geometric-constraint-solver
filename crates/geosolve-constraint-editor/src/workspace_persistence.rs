...
//! Validated native editor workspace checkpoints, including supported historical formats.
//! This codec owns no storage, browser, file publication or source-history policy.
...
use geosolve_sketch::SolverConfig;
use geosolve_sketch::{
...
pub struct WorkspaceSnapshot {
    ...
    pub revisions: WorkspaceRevisions,
    #[serde(skip)]
...
struct WorkspaceLegacyBootstrap<'a> {
    source_version: u32,
...
impl WorkspaceLegacyBootstrap<'_> {
    ...
    fn design_document(&self) -> Result<SketchDocument, String> {
        self.snapshot.design_document()
    }

    fn accepted_document(&self) -> Result<Option<SketchDocument>, String> {
        self.snapshot.accepted_document()
    }

    fn feature_document(&self) -> Result<ComputedFeatureDocument, String> {
        self.snapshot.feature_document()
    }

    const fn revisions(&self) -> SketchLifecycleRevisionHighWater {
        self.snapshot.revisions()
    }

    const fn accepted_belongs_to_current_design(&self) -> bool {
        self.snapshot.accepted_belongs_to_current_design
    }

    const fn sketch_identity_high_water(&self) -> &SketchPersistentIdentityHighWater {
        &self.snapshot.sketch_identity_high_water
    }

    const fn feature_lifecycle_high_water(&self) -> ComputedFeatureLifecycleHighWater {
...
enum WorkspaceDocumentEncoding {
    CanonicalV4,
    DraftV5,
}
...
impl WorkspaceSnapshot {
    ...
    ///
    /// # Errors
    /// Rejects missing or inconsistent accepted native authority.
    pub fn projectional_authority_metadata(
    ...
    ///
    /// # Errors
    /// Rejects unencodable or invalid retained design, geometry or feature state.
    pub fn from_coordinator(coordinator: &RetainedEditorCoordinator) -> Result<Self, String> {
    ...
    ///
    /// # Errors
    /// Rejects unavailable projectional authority or invalid checkpoint contents.
    pub fn from_projectional_editor(
        projectional: &ProjectionalEditorSession,
    ) -> Result<Self, String> {
    ...
    ///
    /// # Errors
    /// Rejects inconsistent persistent allocator/revision bounds or invalid native authority.
    pub fn from_projectional_editor_with_persistence_high_water(
        projectional: &ProjectionalEditorSession,
        computed_evaluation_high_water: ComputedEvaluationAllocatorHighWater,
    ...
    fn from_delegated_projectional_editor(
        projectional: &ProjectionalEditorSession,
        computed_evaluation_high_water: ComputedEvaluationAllocatorHighWater,
        retained_revisions: WorkspaceRevisions,
    ) -> Result<Self, String> {
    ...
    ///
    /// # Errors
    /// Rejects invalid delegated authority or checkpoint serialization beyond the wire limits.
    pub fn encode_delegated_projectional_editor(
        projectional: &ProjectionalEditorSession,
        computed_evaluation_high_water: ComputedEvaluationAllocatorHighWater,
        retained_revisions: WorkspaceRevisions,
    ...
    ///
    /// # Errors
    /// Rejects inconsistent versioned authority or serialization beyond the wire limits.
    ...
    ///
    /// # Errors
    /// Rejects unsupported versions, malformed, oversized or inconsistent checkpoint authority.
    ...
    ///
    /// # Errors
    /// Rejects an invalid retained intent checkpoint.
    pub fn intent_session(&self) -> Result<Option<IntentSession>, String> {
        #[cfg(test)]
    ...
    ///
    /// # Errors
    /// Rejects absent, corrupt or nondelegated intent authority.
    pub fn validate_delegated_intent_checkpoint(&self) -> Result<(), String> {
        #[cfg(test)]
        PROJECTIONAL_INTENT_DECODES.with(|count| count.set(count.get() + 1));
    ...
    /// Whether this checkpoint requires historical flat-to-intent normalization.
    #[must_use]
    pub fn is_legacy(&self) -> bool {
        self.legacy_bootstrap().is_some()
    }

    fn legacy_bootstrap(&self) -> Option<WorkspaceLegacyBootstrap<'_>> {
    ...
    fn annotation_layout(&self) -> AnnotationLayoutState {
...
///
/// # Errors
/// Rejects projectional snapshots, invalid geometry, feature authority or retained revision bounds.
pub fn coordinator_from_snapshot(
...
///
/// # Errors
/// Rejects inconsistent intent/native ownership, geometry, branches or validation evidence.
pub fn projectional_editor_from_snapshot(
    snapshot: &WorkspaceSnapshot,
...
///
/// # Errors
/// Rejects unsupported historical data or failed native normalization/validation.
pub fn projectional_editor_from_legacy_snapshot(
    snapshot: &WorkspaceSnapshot,
) -> Result<ProjectionalEditorSession, String> {
...
///
/// # Errors
/// Rejects unavailable or invalid flat native authority and failed intent normalization.
pub fn projectional_editor_from_flat_coordinator(
...
fn layout_form_is_compatible(
    entry: AnnotationLayoutEntry,
    annotation: &crate::SceneAnnotation,
) -> bool {
    match (
        &annotation.geometry,
...
#[cfg(test)]
fn reproduction_payload_from_coordinator(
    coordinator: &RetainedEditorCoordinator,
) -> Result<String, String> {
...
#[cfg(test)]
fn reproduction_payload_from_snapshot(mut snapshot: WorkspaceSnapshot) -> Result<String, String> {
...
#[cfg(test)]
fn coordinator_from_reproduction_payload(
    payload: &str,
) -> Result<RetainedEditorCoordinator, String> {
...
#[cfg(test)]
fn snapshot_from_reproduction_payload(payload: &str) -> Result<WorkspaceSnapshot, String> {
    let workspace =
...
mod tests {
    ...
    fn m83_migrated_v6_continues_through_edit_undo_redo_and_cold_v8_reload_body() {
        ...
        let bootstrap_x_leaf = projectional
            ...
            .ownership
            .writable_leaf(crate::IntentNativeWritableLeaf::PointX { point: historical })

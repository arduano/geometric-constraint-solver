...
/// Maximum semantic object/array segments in one projection path.
///
/// This matches the bounded TypeScript/RPC reader and is deliberately much
/// larger than every path generated by the current closed declaration table.
pub const MAX_INTENT_PROJECTION_PATH_SEGMENTS: usize = 32;
...
/// One structural segment in a schema-derived projection path.
///
/// Field segments name semantic object members. Index segments are used only
/// for genuinely ordered, variable- or repeated-cardinality values. This is a
/// projection coordinate rather than graph authority: exact mutations still
/// resolve to stable slots, fields, ports, and leaves.
#[derive(Clone, Debug, Deserialize, Eq, Hash, Ord, PartialEq, PartialOrd, Serialize)]
#[serde(untagged)]
pub enum IntentProjectionPathSegment {
    Field(IntentKey),
    Index(u16),
}

/// Typed semantic path shared by Structured Source, Inspector, and code-query
/// projections.
///
/// The JSON representation is an ordinary array such as
/// `["corners", 0, "parents", 1, "parameter"]`; padded pseudo-fields and
/// string-encoded array selectors are deliberately absent.
#[derive(Clone, Debug, Eq, Hash, Ord, PartialEq, PartialOrd, Serialize)]
#[serde(transparent)]
pub struct IntentProjectionPath(Vec<IntentProjectionPathSegment>);

impl<'de> Deserialize<'de> for IntentProjectionPath {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        let segments = Vec::<IntentProjectionPathSegment>::deserialize(deserializer)?;
        validate_projection_path_segments(&segments).map_err(serde::de::Error::custom)?;
        Ok(Self(segments))
    }
}

fn validate_projection_path_segments(
    segments: &[IntentProjectionPathSegment],
) -> Result<(), &'static str> {
    if segments.is_empty() {
        return Err("intent projection path must not be empty");
    }
    if segments.len() > MAX_INTENT_PROJECTION_PATH_SEGMENTS {
        return Err("intent projection path exceeds its segment bound");
    }
    if !matches!(
        segments.first(),
        Some(IntentProjectionPathSegment::Field(_))
    ) {
        return Err("intent projection path must begin with an object field");
    }
    Ok(())
}

impl IntentProjectionPath {
    /// Constructs a non-empty path from already validated semantic segments.
    ///
    /// # Panics
    ///
    /// Panics when `segments` is empty. Every schema coordinate must name at
    /// least one field or indexed collection member.
    #[must_use]
    pub(crate) fn new(segments: Vec<IntentProjectionPathSegment>) -> Self {
        validate_projection_path_segments(&segments)
            .expect("source-controlled intent projection path is valid");
        Self(segments)
    }

    #[must_use]
    pub fn field(field: IntentKey) -> Self {
        Self(vec![IntentProjectionPathSegment::Field(field)])
    }

    #[must_use]
    pub fn indexed(collection: IntentKey, index: u16) -> Self {
        Self(vec![
            IntentProjectionPathSegment::Field(collection),
            IntentProjectionPathSegment::Index(index),
        ])
    }

    #[must_use]
    pub fn segments(&self) -> &[IntentProjectionPathSegment] {
        &self.0
    }

    /// Appends an object field to this semantic path.
    ///
    /// # Panics
    ///
    /// Panics when appending the field would exceed
    /// [`MAX_INTENT_PROJECTION_PATH_SEGMENTS`].
    #[must_use]
    pub fn with_field(mut self, field: IntentKey) -> Self {
        assert!(
            self.0.len() < MAX_INTENT_PROJECTION_PATH_SEGMENTS,
            "intent projection path exceeds its segment bound"
        );
        self.0.push(IntentProjectionPathSegment::Field(field));
        self
    }

    /// Appends an array index to this semantic path.
    ///
    /// # Panics
    ///
    /// Panics when appending the index would exceed
    /// [`MAX_INTENT_PROJECTION_PATH_SEGMENTS`].
    #[must_use]
    pub fn with_index(mut self, index: u16) -> Self {
        ...
        self.0.push(IntentProjectionPathSegment::Index(index));
        self
    }
}

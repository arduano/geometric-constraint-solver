...
pub enum IntentModelError {
    ...
    #[error("materialization evidence digest mismatch")]
...
impl IntentNodeDraft {
    ...
    /// Returns the exact bounded number of stable ports generated by this
    /// draft's closed schema before authored inputs decide native reservation
    /// reuse.
    ///
    /// This query is intended for transport/resource preflight. It returns
    /// `None` for a child/output shape which the graph resource contract will
    /// reject, before expanding an attacker-controlled operation span count.
    /// It does not validate required fields or inputs and allocates no stable
    /// identity.
    #[must_use]
    pub fn schema_generated_port_count(&self) -> Option<usize> {
        let child_ports =
            schema_generated_child_port_count(&self.kind, self.dynamic_children, &self.fields)?;
        if let IntentNodeKind::Operation { operation } = &self.kind {
            if self.operation_outputs.len() > MAX_INTENT_OPERATION_OUTPUTS
                || matches!(
                    *operation,
                    OperationKind::Split
                        | OperationKind::Break
                ...
                ) && !self.operation_outputs.is_empty()
            {
                return None;
            }
            let mut spans = 0_usize;
            let mut paired_sources = 0_usize;
            for output in &self.operation_outputs {
                if (output.kind == IntentOperationOutputKind::Curve)
                    != (output.curve_span_count > 0)
                {
                    return None;
                }
                spans = spans.checked_add(usize::from(output.curve_span_count))?;
                if matches!(
                    output.kind,
                    IntentOperationOutputKind::Constraint | IntentOperationOutputKind::Dimension
                ) {
                    paired_sources = paired_sources.checked_add(1)?;
                }
            }
            ...
            return 2_usize
                .checked_add(self.operation_outputs.len())?
                .checked_add(paired_sources)?
                .checked_add(spans)?
                .checked_add(child_ports);
        }
        if !self.operation_outputs.is_empty() {
            return None;
        }
        schema_generated_node_port_count(&self.kind, self.dynamic_children, &self.fields)?
            .checked_add(child_ports)
    }
...
pub(crate) const COMPUTED_FILLET_FIELDS_PER_CORNER: usize = 22;

fn schema_generated_child_port_count(
    kind: &IntentNodeKind,
    dynamic_children: u16,
    fields: &BTreeMap<IntentFieldKey, IntentLiteral>,
) -> Option<usize> {
    let children = usize::from(dynamic_children);
    if children > MAX_INTENT_NODE_CHILDREN {
        return None;
    }
    match kind.child_schema() {
        IntentChildSchema::None => (children == 0).then_some(0),
        IntentChildSchema::PolylineVertex => {
            if !(2..=MAX_INTENT_NODE_CHILDREN).contains(&children) {
                return None;
            }
            let spans = if definition_boolean(fields, "closed").unwrap_or(false) {
                children
            } else {
                children.checked_sub(1)?
            };
            children.checked_add(spans)
        }
        ...
        IntentChildSchema::FilletCorner => {
            ...
            (children > 0 && children <= maximum).then_some(children)
        }
        IntentChildSchema::PatternInstance => {
            (children > 0 && children <= MAX_INTENT_NODE_CHILDREN).then_some(children)
        }
    }
}

fn schema_generated_node_port_count(
    kind: &IntentNodeKind,
    dynamic_children: u16,
    fields: &BTreeMap<IntentFieldKey, IntentLiteral>,
) -> Option<usize> {
    match kind {
        IntentNodeKind::Geometry { recipe } => {
            schema_generated_geometry_node_port_count(*recipe, dynamic_children, fields)
        }
        IntentNodeKind::Constraint { constraint } => {
            usize::from(constraint_contact_count(*constraint))
                .checked_mul(2)?
                .checked_add(2)
        }
        IntentNodeKind::Dimension { .. } => Some(3),
        IntentNodeKind::Operation { .. } => unreachable!("operations are counted separately"),
        ...
        | IntentNodeKind::Identity { .. } => Some(1),
        IntentNodeKind::Bootstrap { object } => Some(match object.kind {
            BootstrapNativeKind::Curve
            | BootstrapNativeKind::Constraint
            | BootstrapNativeKind::Dimension => 2,
            ...
            | BootstrapNativeKind::AnnotationPlacement => 1,
        }),
    }
}

fn schema_generated_geometry_node_port_count(
    recipe: GeometryRecipeKind,
    dynamic_children: u16,
    fields: &BTreeMap<IntentFieldKey, IntentLiteral>,
) -> Option<usize> {
    ...
    let authored: usize = match recipe {
        ...
        | G::Parabola
        | G::CubicBezier => 4,
        G::CenterRectangle | G::ThreePointCircle | G::AxisEndpointsEllipse | G::Hyperbola => 5,
        G::ThreePointCenterRectangle => 6,
        G::CenterArc | G::ThreePointArc | G::TangentArc => 7,
        G::CenterAxesEllipticalArc => 8,
        G::AxisEndpointsEllipticalArc => 9,
    };
    let curves: usize = match recipe {
        G::SketchPoint => 0,
    ...
    let relations = match recipe {
        G::MidpointLine | G::TangentArc => 1,
        G::TwoPointAlignedRectangle | G::ThreePointCenterRectangle => 4 + usize::from(regularized),
        G::ThreePointCornerRectangle => 3 + usize::from(regularized),
        G::CenterRectangle => 5 + usize::from(regularized),
        _ => 0,
    };
    spans
        .checked_add(authored)?
        .checked_add(curves)?
        .checked_add(usize::from(recipe == G::Polyline))?
        .checked_add(if recipe == G::TangentArc { 4 } else { 0 })?
        .checked_add(relations.checked_mul(2)?)
}
...
impl IntentExternalInputs {
    ...
    /// Revalidates the resource bounds of a decoded host-input snapshot.
    ///
    /// Constructors are not invoked by Serde, so canonical session import
    /// calls this method for current and historical checkpoints.
    ///
    /// # Errors
    ///
    /// Returns an error when either opaque component exceeds its bound.
    pub fn validate(&self) -> Result<(), IntentModelError> {
        validate_component("parameter batch", &self.parameter_batch)?;
        validate_component("external snapshots", &self.external_snapshots)
    }
...
impl MaterializationEvidence {
    ...
    /// Revalidates decoded host artifacts and their content digest.
    ///
    /// # Errors
    ///
    /// Returns an error when an opaque component exceeds its bound or when
    /// the decoded digest does not authenticate the retained artifacts.
    pub fn validate(&self) -> Result<(), IntentModelError> {
        validate_component("materialization", &self.materialization)?;
        validate_component("ownership map", &self.ownership)?;
        validate_component("host validation artifact", &self.host_validation)?;
        if self.digest != materialization_evidence_digest(self) {
            return Err(IntentModelError::EvidenceDigestMismatch);
        }
        Ok(())
    }

    pub(crate) fn reauthenticate(&mut self) {
        self.digest = materialization_evidence_digest(self);
    }
}

fn materialization_evidence_digest(evidence: &MaterializationEvidence) -> ContentDigest {
    digest_bytes(
        &serde_json::to_vec(&(

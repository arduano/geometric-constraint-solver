...
impl OperationController {
    ...
    /// Creates a cooperatively cancellable child envelope from the exact work still available.
    /// A caller must merge the child's final report with [`Self::absorb_child_report`] before it
    /// can publish the compound operation's result.
    #[must_use]
    pub fn child_control(&self) -> OperationControl {
        OperationControl::new(
            self.control.token.clone(),
            self.control.limits.remaining(self.consumed),
        )
    }

    /// Merges one completed or stopped child operation into this compound operation.
    ///
    /// Additive counters are accumulated, dense dimensions retain their maximum, and the child's
    /// first typed stop reason becomes the compound operation's stop reason. A report that exceeds
    /// the remaining parent envelope fails closed.
    ///
    /// # Errors
    ///
    /// Returns the child stop reason or a typed work-exhaustion reason if its report cannot fit.
    #[allow(
        clippy::too_many_lines,
        reason = "the closed counter inventory keeps compound work-accounting reviewable and exhaustive"
    )]
    pub fn absorb_child_report(
        ...
        let checkpoint =
            report
                ...
                .map_or(
                    ...
                    |reason| match reason {
                        OperationStopReason::Cancelled { checkpoint }
                        | OperationStopReason::WorkExhausted { checkpoint, .. } => checkpoint,
                    },
                );
        for (counter, amount) in [
            (
                OperationWorkCounter::DocumentValidationItems,
                report.consumed.document_validation_items,
            ),
            (
            ...
            (
                ...
                report.consumed.nonlinear_iterations,
            ),
            (
                OperationWorkCounter::RejectedTrials,
                report.consumed.rejected_trials,
            ),
            (
                OperationWorkCounter::ComponentLinearizations,
                report.consumed.component_linearizations,
            ),
            (
                OperationWorkCounter::Factorizations,
                report.consumed.factorizations,
            ),
            (
                OperationWorkCounter::RankKernels,
                report.consumed.rank_kernels,
            ),
            (
                OperationWorkCounter::DiagnosticCandidates,
                report.consumed.diagnostic_candidates,
            ),
            (
                OperationWorkCounter::DiagnosticTrials,
                report.consumed.diagnostic_trials,
            ),
            (
                OperationWorkCounter::ProfileCandidatePairs,
                report.consumed.profile_candidate_pairs,
            ),
            (
                OperationWorkCounter::ProfileSubdivisions,
                report.consumed.profile_subdivisions,
            ),
            (
                OperationWorkCounter::ProfileRoots,
                report.consumed.profile_roots,
            ),
            (
                OperationWorkCounter::ProfileFragments,
                report.consumed.profile_fragments,
            ),
            (
                OperationWorkCounter::ProfileIntegrations,
                report.consumed.profile_integrations,
            ),
            (
                OperationWorkCounter::ProfileContainmentTests,
                report.consumed.profile_containment_tests,
            ),
            (
                OperationWorkCounter::ProfileFaces,
                report.consumed.profile_faces,
            ),
            (
                OperationWorkCounter::MeasurementIntegrations,
                report.consumed.measurement_integrations,
            ),
            (
                OperationWorkCounter::MeasurementDerivativeEvaluations,
                report.consumed.measurement_derivative_evaluations,
            ),
        ] {
        ...
        for (counter, amount) in [
            (
                ...
                report.consumed.dense_kernel_rows,
            ),
            (
                OperationWorkCounter::DenseKernelColumns,
                report.consumed.dense_kernel_columns,
            ),
        ] {
            ...
            self.consumed
                .set(counter, self.consumed.consumed(counter).max(amount));
        }
        if let Some(reason) = report.stopping_reason {
            self.stopping_reason = Some(reason);
            return Err(reason);
        }
        Ok(())
...
mod tests {
    ...
    #[test]
    fn child_reports_compose_remaining_work_and_typed_stops() {
        let mut limits = OperationLimits::unlimited();
        limits.profile_candidate_pairs = 5;
        limits.profile_subdivisions = 3;
        let mut parent =
            OperationController::new(OperationControl::new(CancellationToken::default(), limits));
        parent
            .charge(
            ...
            .unwrap();
        let child_control = parent.child_control();
        assert_eq!(child_control.limits.profile_candidate_pairs, 3);
        let mut child = OperationController::new(child_control);
        child
            .charge(
                OperationWorkCounter::ProfileCandidatePairs,
                3,
        ...
        child
            ...
            .unwrap();
        parent.absorb_child_report(child.report()).unwrap();
        assert_eq!(parent.report().consumed.profile_candidate_pairs, 5);
        assert_eq!(parent.report().consumed.profile_subdivisions, 1);

        let mut exhausted_child = OperationController::new(parent.child_control());
        assert_eq!(
            exhausted_child.charge(
                OperationWorkCounter::ProfileCandidatePairs,
                1,
                OperationCheckpoint::ProfileCandidate,
            ),
        ...
        assert_eq!(
            parent.absorb_child_report(exhausted_child.report()),
            Err(OperationStopReason::WorkExhausted {
                counter: OperationWorkCounter::ProfileCandidatePairs,
                checkpoint: OperationCheckpoint::ProfileCandidate,
            })

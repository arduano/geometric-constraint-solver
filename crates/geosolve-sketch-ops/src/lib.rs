...
/// dimension slots each own their immediately following audit-source identity atomically.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
    ...
    > {
        ...
        Ok(
            session.transact(self.input.design_identity(), move |document| {
                ...
                validate_reserved_application(&application, &expected_application, &output_plan)?;
                Ok(application)
            })?,
        )
    }
...
fn build_result(
    ...
    let output_plan = operation_output_plan(&expected)?;
    scratch.validate()?;
...
fn operation_output_plan(
    ...
    Ok(SketchOperationOutputPlan {
        ...
        slots: kinds
            .into_iter()
            .enumerate()
            .map(|(ordinal, kind)| SketchOperationOutputSlot { ordinal, kind })
            .collect(),
    })
}
...
fn validate_reserved_application(
    actual: &SketchOperationApplication,
    expected: &SketchOperationApplication,
    output_plan: &SketchOperationOutputPlan,
) -> Result<(), DocumentError> {
    ...
    let expected_retained = expected
        ...
        .cloned()
        .collect::<Vec<_>>();
    if actual.kind != expected.kind || actual_retained != expected_retained {
        return Err(operation_replay_error(
            "reserved replay changed the authenticated retained/replaced identity disposition",
        ));
    }
    if operation_output_plan(actual)? != *output_plan {
        return Err(operation_replay_error(
            "reserved replay changed the authenticated generated-output inventory",
        ));
    }
    Ok(())
}

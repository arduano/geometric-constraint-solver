...
export default sketch(($) => {
  ...
  const cavityWallLength = $.dimension.curveLength("cavityWallLength", {
    curve: section.segments.byKey.rightLipSupportWall,
    value: mm(11.15),
  });
  ...
  const plan = $.geometry.twoPointAlignedRectangle("plan", {
    firstCorner: [-20.75, 33],
    oppositeCorner: [20.75, 74.5],
    label: "1 x 1 Gridfinity plan envelope",
    role: "profile",
  });
  const planAnchor = $.constraint.fixedPoint("planAnchor", {
  ...
  $.group("1 x 1 plan study", [plan, planAnchor, planWidth, planDepth, planDiagonal, planCenter, planCenterAtMidpoint, labelPocket, labelPocketRadius]);
  return {};
});

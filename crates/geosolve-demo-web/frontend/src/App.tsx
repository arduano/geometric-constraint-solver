    ...
    <div className="flex h-dvh min-h-[720px] min-w-[1024px] flex-col overflow-hidden bg-canvas text-foreground">
      ...
      <div className="relative flex min-h-0 flex-1">
        ...
        <PanelGroup direction="horizontal" autoSaveId="geosolve-workbench-shell-v2" storage={panelStorage}>
          ...
          {detailsOpen && mode !== "code" && <><ResizeHandle /><Panel id="details" defaultSize={21} minSize={18} maxSize={34}><DetailsPanel metadataActions={metadataActions} metadataBlockedReason={metadataBlockedReason} snapshot={snapshot} navigationBlockedReason={(capturedGesture || activeTool !== "select" || Boolean(snapshot.pendingManagedMutation)) ? "Finish the current tool or gesture before navigating between views." : historyBlocked ? "Apply or Revert the source draft before navigating between views." : snapshot.navigation && !snapshot.navigation.canNavigateSource ? snapshot.navigation.unavailableReason ?? "Source navigation is unavailable." : undefined} onOpenCode={openSelectionInCode} onParameterEdit={(id, value) => { if (localDraftDirty) return; void command("parameter.edit", { id, value }); }} onProblemOpen={openProblem} parametersBlocked={localDraftDirty || snapshot.source.dirty} dimensionInspectionBlocked={metadataPending ? "Applying document properties…" : (capturedGesture || activeTool !== "select" || Boolean(snapshot.pendingManagedMutation)) ? "Finish the current tool or gesture before inspecting dimensions." : undefined} dimensionActions={{ onFocus: (id) => { void command("dimensions.focus", { id }); }, onPin: (id, pinned) => { void command("dimensions.pin", { id, pinned }); }, onClearPins: () => { void command("dimensions.clearPins"); }, onEdit: (id, value) => { if (localDraftDirty || snapshot.source.dirty) return; void command("dimensions.edit", { id, value }); } }} /></Panel></>}
        </PanelGroup>
      </div>
...
function CodeWorkspace({ metadataActions, metadataBlockedReason, mode, surface, setSurface, snapshot, selectedFile, draft, setDraft, command, navigation, onShowInCanvas, navigationBlockedReason, declarationActions, declarationBlockedReason, onParameterEdit, onProblemOpen }: { metadataActions: AuthoringMetadataActions; metadataBlockedReason?: string; mode: WorkspaceMode; surface: CodeSurface; setSurface: (surface: CodeSurface) => void; snapshot: WorkbenchSnapshot; selectedFile: WorkbenchSnapshot["source"]["files"][number]; draft: string; setDraft: (value: string) => void; command: (name: string, payload?: unknown) => Promise<WorkbenchSnapshot>; navigation: EditorNavigation | null; onShowInCanvas: (range: Utf16SourceRange) => void; navigationBlockedReason?: string; declarationActions: DeclarationPanelActions; declarationBlockedReason?: string; onParameterEdit: (id: string, value: string) => void; onProblemOpen: (problem: WorkbenchSnapshot["problems"][number]) => void }) {

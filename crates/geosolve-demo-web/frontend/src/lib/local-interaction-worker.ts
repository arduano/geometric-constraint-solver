...
export type LocalInteractionMethod = "construct" | "replace" | "dispatch" | "pointer" | "wheel" | "resize" | "cancel" | "state";
    ...
    const run = async () => {
      try {
        ...
        if (data.method === "construct") {
          ...
          else if (["replace", "dispatch", "pointer", "wheel", "resize", "cancel"].includes(data.method)) {

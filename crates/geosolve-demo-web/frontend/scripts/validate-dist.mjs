...
if ((fileSizes.get(wasmFile) ?? 0) >= maximumReleaseWasmBytes) {
  throw new Error(`${wasmFile} must remain below the 20 MiB optimized-release ceiling`);
}
...
if (totalBytes >= maximumDistributionBytes) {
  throw new Error("distribution must remain below the 30 MiB optimized-release ceiling");
}

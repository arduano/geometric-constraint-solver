...
use geosolve_sketch_code::managed_control_label;

use std::str::FromStr as _;

pub(crate) mod accepted_browsing;
pub(crate) mod chrome_read;
use chrome_read::{ChromeRead, CodeChrome};
...
use geosolve_constraint_editor::presentation_persistence::{
    WorkbenchPersistenceEnvelope, WorkbenchPresentationPersistence,
};
...
impl WorkbenchBridge {
    ...
    fn restore(encoded: &str) -> Result<Self, String> {
        ...
        if let Ok(workspace) = geosolve_constraint_editor::reproduction::decode_workspace(encoded) {
    ...
    pub(crate) fn reproduction_json(&self) -> Result<String, String> {
        let contents = geosolve_constraint_editor::reproduction::encode_workspace(
            &self.persistence_contents()?,
        )
        .map_err(|error| error.to_string())?;
    ...
    fn declaration_row_target(&self, id: &str) -> Option<DeclarationRowTarget> {
        self.chrome_read().declaration_row_target(id)
    }
    ...
    fn declaration_move_mutation(
        ...
        self.chrome_read().declaration_move_mutation(payload)
    }

    fn declaration_drop_target(&self, id: &str) -> Result<String, String> {
        self.chrome_read().declaration_drop_target(id)
    }
    ...
    fn suppression_source_mutation(
        ...
        self.chrome_read()
            .suppression_source_mutation(id, suppressed)
    }
    ...
    fn parameter_source_mutation(
        ...
        self.chrome_read().parameter_source_mutation(id, value)
    }
    ...
    fn managed_row_target(&self, id: &str) -> Result<ManagedMutationTarget, String> {
        self.chrome_read().managed_row_target(id)
    }
    ...
    fn explorer_scene_items(
        ...
        self.chrome_read().explorer_scene_items(scene, rows)
    }
    ...
    fn problem_snapshot(&self) -> Vec<ProblemSnapshot> {
        ...
        self.chrome_read().append_native_problems(&mut problems);
...
mod tests {
    ...
    fn m90_f005_reproduction_bridge() -> WorkbenchBridge {
        const FIXTURE: &str = include_str!(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../geosolve-constraint-editor/tests/fixtures/m90_f005_native_drag_repro.txt"
        ));
        ...
        let workspace =
            geosolve_constraint_editor::reproduction::decode_workspace(FIXTURE.trim_end())
...
"#;
    ...
    fn m92_legacy_envelope(bridge: &WorkbenchBridge) -> String {
        ...
        code["session"] = geosolve_constraint_editor::reproduction::decode_workspace(
            code["session"].as_str().unwrap(),
        )
        .unwrap()
        .into();
    ...
    fn m92_exact_captured_legacy_workspace_recovers_complete_history() {
        ...
        assert_eq!(
            geosolve_constraint_editor::reproduction::decode_workspace(
                code["session"].as_str().unwrap()
            )
            .unwrap(),

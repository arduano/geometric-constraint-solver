...
4. Apply the candidate, drag or edit a parent, edit the resulting radius dimension, then Undo and
   Redo.
...
parent trims and one editable driving radius; parent edits remain associative. Unsupported or
unresolved local roots show a typed warning and retain the prior accepted scene. First Escape
clears the candidate; a second Escape exits Fillet mode.
...
5. Undo/Redo both operations and attempt the tool on a circle or nonlinear curve.
...
freedom while retaining its signed distance. The created dimension remains an ordinary editable
driving dimension. Unsupported source families warn without approximation or allocation.
...
5. Try an unsupported circle, conic or NURBS source and cancel a partially collected mirror.
...
editable operation output; no feature-specific guide, protected mode, old harness or `/#/dev/lab`
appears.

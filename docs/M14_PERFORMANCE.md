...
The native import/solve/edit boundaries remain current release gates. Browser
boundaries and measurements below are explicitly historical; M50 retired their
automation without weakening solver correctness or the native budgets.
...
- Historical browser render started with accepted WASM state and measured one zoom action, complete SVG/inspector DOM replacement, and synchronous event handling.
- Native measurements use two warmups and 12 samples. The retired browser harness used 12 alternating zoom samples. The recorded statistic is p95.
...
| Browser render (historical; retired by M50) | 75 ms | 400 ms |

The native limits are alpha interaction ceilings rather than solver
microbenchmark targets. A native budget failure must be fixed by implementation
or by explicitly reducing the supported alpha document envelope, never by
weakening correctness policy. The browser row records the retired checkpoint only.
...
The surviving release build remains available from `crates/geosolve-demo-web`:

```bash
nix-shell ../../shell.nix --run 'trunk build --release'
```

The deleted M14 Chromium suite historically used Node standard-library APIs and raw DevTools
Protocol. M50 removed that browser gate after direct semantic replacement; the measurements
above and burst contract below remain historical evidence and are not runnable
post-cleanup instructions.

The retired browser gate also dispatched a 40-event desktop pointer burst against
the five-stage scissor tower. At that checkpoint the burst had to coalesce into
exactly one animation-frame render within 100 ms, retain validated projected
geometry, defer detailed audit DOM updates only while the drag was active and
flush the final target before release.

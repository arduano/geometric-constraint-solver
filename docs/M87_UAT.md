...
Status: **OPEN pending M87-U9/U10 focused human visual UAT**. The supervising user's earlier
2026-08-30 scoped disposition remains historical acceptance of M87-U1 through M87-U8 without a
separate row-by-row replay. It does not accept the later manufacturing visual rows, and M87 remains
open. The retained sound work and complete adaptive-detail/LOD removal are unchanged. M88 is
inactive.

## Candidate identity

No twelve-project release candidate was nominated. The shared worktree is dirty and no commit is
authorized; the mutable Tailscale development server is useful collaboration evidence, not
candidate or publication authority. The complete dirty-worktree mechanical gate passed at exit
`0` on 2026-08-30 for the pre-M87-F003 sources with:
...
That gate does not qualify the revised fully constrained samples. The current reviewed twelve-row
code-project ledger has SHA-256
`f6ecd037cef8befc59f9a057fef499a14f0851f8ec5a0d3a1468a69e66a9d1bc`. This evidence does not create
a clean source/tree, no-rebuild freeze, immutable candidate, public deployment or service-retirement
claim. The post-F003 manufacturing owner suite passes 3/3, native composition passes 13/13, the
reviewed-ledger check passes 1/1 and the all-demo headless/deterministic-product suite passes 10/10.
Fresh mutable bundles awaiting human review are:
...
amendment created two additional visual rows, and no subsequent human disposition accepts them.
M87-F003 then replaced literal/fixed-lock authority with fully constrained relational designs; the
old visual bundles do not review that repair. Proportional post-F003 mechanical qualification is
complete, and M87 remains open at the focused U9/U10 boundary. No clean candidate, immutable artifact,
public deployment or service-retirement result is inferred; the existing Tailscale listener remains
a mutable development service. Performance concerns are deferred to a follow-up milestone and M88
remains inactive.

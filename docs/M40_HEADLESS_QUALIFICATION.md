...
| Canvas/tree/Inspector selection coherence, including UAT-C1-F3 multi-select and line picking | `line_is_selected_from_screen_space_without_dom_hit_targets`, `point_has_priority_at_a_line_endpoint`, `extended_line_selection_resolves_and_applies_contextual_parallel_relation`, and the click-threshold test cover line tolerance, priority, ordered Shift selection and a contextual Parallel application. | No executed parity evidence. | `e2e/m40.mjs` creates/selects two points with Shift and applies Coincident; it asserts persistent tree IDs/no index selection. It does not exercise curve selection, tree-to-canvas coherence, Ctrl/Command, overlap ties, or Inspector. | **Partial.** Add generated hit/selection sequences (replace/toggle, Shift/Ctrl/Command, stale/deleted selection), exact 6.5/7 px and overlap/tie vectors across viewports/scales. Run identical vectors in WASM. Add browser event/render cases for curve selection and tree selection identity reflection only. |
| Constraint application and persistent glyph (fixed, coincident, horizontal, vertical, parallel, perpendicular, equal length) | `every_resolved_relation_executes_through_the_authoring_adapter` covers the complete contextual relation family; coordinator `contextual_authoring_resolution_is_prospective_until_one_coordinator_apply`, stale-effect tests, and persistent-ID delete/suppression tests own prospective application and lifecycle behavior. `complete_relation_and_dimension_matrix_is_headless_and_selection_scoped`, `m71_contextual_relation_availability_rejects_semantic_tautologies` and `m71_f002_contextual_relation_availability_rejects_missing_objects_and_invalid_spans` own the retained typed invalid-selection slices. | No executed parity evidence. | `e2e/m40.mjs` applies only Coincident and sees one glyph. Current `mod.rs` maps stable action identities to `AuthoringTool`, drives `AuthoringState`, and sends the resulting `AuthoringApplication` through `RetainedEditorCoordinator::apply_authoring`; `canvas_authoring_click_sequence_contributes_one_operand_and_rearms_after_terminal_attempt` is the focused adapter owner. | **Partial.** Add table-driven native applicability/effect/replay vectors for all seven kinds, valid/invalid arity/type/span and retained rejection. Run them in WASM. Browser should only prove widget-key-to-typed-action dispatch and glyph identity for representative point, one-line, and two-line action paths. |

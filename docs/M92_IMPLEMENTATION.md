...
Status: **in progress; 16-sample pruning qualification and replacement immutable nomination
pending. Earlier audited gate and nomination receipts remain historical. Human U1–U8 remain
...
Renewed qualification must cover all 16 browser workflows and 32 meaningful source edits, eight
...
catalog count; frozen endpoint checks retain 19 ordinary rows plus the separate Jansen workflow.
The previous immutable 20-sample snapshot remains served at `http://100.94.63.83:18092/` until a
qualified replacement is frozen and verified. No M92 closure, human-row disposition or Pages
publication follows from this pruning authorization.
...
These are the implementation and test owners. Fresh command outcomes and replacement artifact
identity remain pending. The qualification ledger below preserves pre-pruning evidence.

...
The maintainer removed `prusa-mini-interface`, `nema-17-motor-interface`,
...
pruned-menu/Recent checks. The qualified replacement now serves the archived preview;
the old listener was retired only after local verification and its immutable bytes are retained.
The pruning authorization itself did not close M92; the later maintainer sign-off recorded
...
`geosolve-m92-playwright-rerun.xGmpatAc/playwright.log`, SHA-256
...
`20/20` rows. A production frozen distribution deliberately omits that page, so each local/preview
...
`geosolve-m92-release-gate-r2.O2RJc0L5/release-gate.log`, SHA-256
...
Its no-rebuild ten-file snapshot `geosolve-m92-uat._8s63qiy`, external manifest SHA-256
`2dd2173040aebd99227aba488e2992ef864dd44ed2e9c95f709d5fc7884b20a9`, is nominated at
the archived preview. All 16 browser sample workflows, 32 measured edits and eight public
...
NO_COLOR=true GEOSOLVE_CHROMIUM_PATH=$(command -v google-chrome) \
...
maintainer approval after the composite UAT.

...
clean-qualified M75-F001 replacement remain historical evidence only. the maintainer accepted the
...
`geosolve-m75-f001-uat.2Ju7gq`. The directory is mode `0555`; its seven regular non-symlink
files are mode `0444`:
...
aggregate. Evidence is retained at `geosolve-m75-f001-http-verify.kXc5g5`. The unchanged M72
...
Focused Chromium script `m75_f001_browser_check.mjs`, SHA-256
...
`geosolve-m75-f002-uat.hlSQYT`. The directory is mode `0555`; all seven entries are regular
non-symlink files at mode `0444`:

The archived manifest records the per-file sizes and checksums.

The unchanged M72 and M74 scripts pass over preview at both `1440x900` and `1024x720`; their
...
alone disposed no human item. The maintainer subsequently reported the candidate looking
...
`geosolve-m75-pages-verify.NkQwem/github-pages.zip`. The ZIP is 2,108,111 bytes with SHA-256
...
M76 now supersedes it. The frozen M75 preview snapshot remains accepted historical candidate
evidence.
...
### Initial immutable preview candidate

The final successful gate's exact seven files were copied without rebuilding to
`geosolve-m75-uat.hUSaG7`. The directory is mode `0555`; every entry is a regular,
...
evidence is retained at `geosolve-m75-http-verify.sQGN1B`.

The unchanged compatibility scripts pass directly over preview at `1440x900` and `1024x720`:
...
- **Pass:** the complete clean release gate passes and its exact output is the read-only,
  byte-verified preview candidate kept live through follow-up UAT.
- **Pass for scoped closure:** U1-U12 and the focused F001/F002 hover recheck are accepted under the
  maintainer's 2026-08-16 approval. This is not a claim that every detailed step, desktop

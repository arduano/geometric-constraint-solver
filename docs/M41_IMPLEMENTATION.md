...
qualification evidence; M42-M52 subsequently completed and M53 human UAT later passed.
...
- v1-v4 are frozen readers/writers with canonical v4 output. A draft-v5 may be developed but is unsupported until a future schema-freeze milestone and must not silently extend any existing schema (`docs/API_COMPATIBILITY.md` §§Persistence and API tiers).
...
5. **Known limitations/next blocker:** draft-v5 remains deliberately unsupported until a future
   schema-freeze milestone. M42 must add parameter revision/digest input stamps and bindings without

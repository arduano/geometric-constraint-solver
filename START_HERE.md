...
Continue in `PLAN.md` order with M28's generic curve-family fillets and explicit parent trim views. M27 completed the independently validated associative line-line vertical slice, derived accepted arc endpoints, explicit side/order/sweep state, driving/reference radius, sketch JSON v3 and explode/ownership semantics under ADR 0022. Parent lines remain visibly untrimmed and associated output arcs reject executable consumers until M28 differentiates that incidence truthfully. M29 then owns release hardening.

M8 is a contract and benchmark-baseline milestone. Its preserved completion notes and pre-rebaseline M10+ labels in older ADRs and `REFERENCES.md` record the allocation accepted at that time; `PLAN.md` is authoritative for the current M10-M29 numbering.

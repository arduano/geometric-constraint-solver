...
- **Open, not-yet-accepted target:** M87 implements ADR 0042's cohesive managed-control and
  ...
  `docs/M87_HEADLESS.md`, `docs/M87_UAT.md` and ADR 0042 own the open contract. Focused
  manufacturing-owner qualification, all-twelve-demo cold materialization, the reviewed twelve-row
  ledger, shared renderer/headless/browser checks and the recorded full dirty-worktree gate are
  retained mechanical evidence for their exact scopes. The supervising user's earlier 2026-08-30
  scoped disposition remains historical acceptance of U1-U8 without claiming a separate row-by-row
  replay. M87-U9/U10 remain pending focused human visual UAT. No clean exact source/tree is
  nominated, no immutable artifact is frozen, and no public deployment or service retirement is
  performed; the mutable Tailscale listener remains development infrastructure. Performance work is
  deferred to a follow-up milestone; M87 remains open and M88 is not activated here.

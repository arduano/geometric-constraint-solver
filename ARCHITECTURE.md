...
- **Baseline:** implemented and accepted behavior through M88 plus the scoped-approved M90 clean
  break. M89 is retained below as a superseded compatibility-stage record; its row-by-row UAT was
  not run, but M90's completed replacement contract now owns the product boundary. M90 is **closed
  by explicit scoped supervising-user approval on 2026-09-04** after F005/F006 repair, collateral
  ...
  public push was authorized or made. M91's clean-qualified immutable candidate was subsequently
  accepted at milestone level by the supervising user's 2026-09-04 blanket/composite approval:
  M91-U1 through M91-U14 are Pass without claiming a separately logged row-by-row replay. The
  approved descendant is publicly closed and exact-verified on GitHub Pages; the superseded M90/M91
  listeners are retired. M44
...
- **Closed design exploration:** M82 attempted a computed all-family Offset architecture but is not
  ...
  disposition.
- **Completed target:** M83 implements ADR 0040's projectional Design Intent Graph
  ...
  `889f730`, passes clean replacement qualification and immutable Tailscale nomination. The
...
- **Completed target:** M84 implements an optional code/GUI authoring layer under ADR 0041. Managed source
...
- **Completed target:** M85 is a semantic-neutral retained-presentation optimization. M85-F001 routes
  ...
  closed.
- **Completed target:** M86 is a focused bug-fix/UAT batch over the accepted
  ...
  optional code-project owner one closed pre-generic route. It authenticates current Inspector and
  session identity, accepted alias-to-declaration provenance, direct builder family and exact
  target leaf; rewrites only direct curve-length/diameter `target` source lenses; and atomically
  rematerializes through the unchanged code/Intent/native solver pipeline. Accepted publication
  ...
  isolated-HTTP verified. Approval head `ccf791f` passes Pages run `33232073614`, exact artifact and
  hosted-byte verification, and both retained services are retired. M86 is complete;
  `docs/M86_GOALS.md` owns the accepted contract.
- **Completed target:** M87 implements ADR 0042's cohesive managed-control and
  browser-free authoring boundary. `geosolve-sketch-code` derives bounded transient controls from
  parser-owned source spans, declaration schemas, artifact dependency bindings and exact generated
  provenance. A token authenticates project/source/generation and expected typed value; it never
  grants solver authority or enters persisted project, expansion or session formats. Explicit
  non-DoF literals are editable, while solver-instance coordinates, references and structural
  identities remain on their existing typed read-only/draft routes. Shared literals disclose and
  update their complete consumer fan-out in one outer `SketchCodeSession` transaction. `EditLens`
  remains presentation metadata only.

  The optional workbench decorates the ordinary Inspector and Code panel from that same manifest,
  rejects nested code-mode Intent mutations, and exposes a separate strict inspect/edit/Undo/Redo
  code-control RPC. A generated Fillet-radius grip delegates a presentation-independent grouped
  preview to the editor, but release returns an authenticated proposal to the outer source owner;
  no per-frame source parse or nested Intent history is allowed. Existing point overlays and M86
  Select priority are unchanged.

  `geosolve-sketch-render` owns target-neutral camera fitting and SVG composition shared by WASM
  and native hosts, with native-only pure-Rust PNG using a pinned bundled font and no external
  resources. `geosolve-headless` accepts managed-only source, canonical pinned projects or bundled
  demos and performs inspect, exact-CAS edit, cold materialization, independent validation and
  atomic static output without a browser, DOM, server, network, Node or TypeScript execution.

  The additive tenth demonstration is a 360 x 220 mm robotic cable-harness routing board. Eight
  keyed open Polylines remain ordinary native geometry; one data-only `harnessRoute` patch maps
  their keyed vertices/corners to 80 clip circles and 64 existing host Fillets. Shared clip/bend
  literals, invocation-local literals, solver-instance point overlays and keyed structural edits
  retain separate owners. Aggregate validation uses an exact endpoint-connectivity query rather
  than whole-scene visual arrangement, so deliberately intersecting clip circles cannot truncate
  unrelated open routes. Noninteractive code composition translates every Fillet candidate then
  publishes the complete host set atomically, avoiding one preview/publication transaction per
  corner. Renderer/headless consumers see only independently accepted scene authority and remain
  ignorant of patch, control, overlay and reconciliation mechanics.

  The next additive amendment expands that catalog to twelve without adding another engine. The
  eleventh project is a router joinery fit coupon with three 70 mm-wide mortises at respective
  loose/nominal/press heights 18.4/18.0/17.6 mm; its constrained mortises and tabs compose a
  reusable data-only `cornerReliefs` record mapping and the existing selective-Fillet patch. The
  twelfth is one explicit 26-point closed, symmetric Gridfinity 1 x 1 x 3U material section with
  independently composable floor and lip radii. Patch expansion, solver-instance geometry,
  managed controls, keyed reconciliation and rendering retain their existing owners. Both entries
  are 2D/2.5D manufacturing-intent sketches, not CAM, toolpath, automatic cutter compensation,
  boolean, solid or production-manufacturing authority.

  `docs/M87_AUDIT.md`, `docs/M87_GOALS.md`, `docs/M87_IMPLEMENTATION.md`,
  `docs/M87_HEADLESS.md`, `docs/M87_UAT.md` and ADR 0042 own the completed contract. Focused
  manufacturing-owner qualification, all-twelve-demo cold materialization, the reviewed twelve-row
  ledger and shared renderer/headless/browser checks pass. Exact source `32c7289`, tree `38f7175`,
  passes the complete clean release gate. The supervising user's 2026-08-31 close decision accepts
  U9/U10 at milestone level without claiming a separate row-by-row replay. No immutable artifact is
  frozen, and no public deployment or service retirement is performed; the mutable Tailscale
  listener remains development infrastructure.
- **Completed target (accepted 2026-09-01):** M88 is a presentation/
  workflow redesign over the unchanged M87 domain and authoring authority. `geosolve-demo-web` owns
  a presentation-only workspace shell with
  central Design/Split/Code modes, resizable panes, compact app bar, primary tool rail, searchable
  start/open surface, collapsible Explorer and Inspector/Parameters/Problems tabs. Layout state may
  select presentation surfaces and sizes but cannot solve, expand, publish history, save semantic
  workspace state or authenticate any domain/code mutation.

  The code editor becomes stable live DOM rather than markup regenerated by every durable render.
  Selected file, exact dirty draft bytes, cursor, selection and scroll survive selection, scene,
  mode and pane changes. Apply/Revert remains the outer `SketchCodeSession` transaction; source-
  positioned Problems retain the previous accepted canvas on invalid input. “Open in code” resolves
  an exact managed owner and range rather than a copied consumer or first invocation.

  The browser and `geosolve-headless` continue to exchange canonical `project.json` plus
  `sketch.ts`; neither renderer nor layout layer executes TypeScript, duplicates equations or owns
  accepted-scene validation. M88 requires no stateful AI/chat service. Full-detail rendering remains
  mandatory and the deleted LOD policy is not restored.

  Before broad UI work, M88 hardens the diagnosed Gridfinity path in ownership order: explicit
  release-mode UAT and WASM stack contract; one-MiB edit reproduction/regression and reduced stack
  use; removal of duplicate checkpoint restoration/validation and no-op post-open encoding; then a
  core full-row-rank redundancy shortcut only when numerical rank already equals all Hard rows.
  Independent residual, domain and branch validation remains mandatory. The native one-MiB stack
  regression, actual-WASM Gridfinity contract, full-row-rank core regression, all-feature workspace
  suite and optimized five-run budgets pass. Historical read-only Rust-DOM snapshot
  ...
  transient listener disappeared.

  React UAT resolved semantic-label M88-F001 and toolbar-hierarchy M88-F002, then M88-F003 exposed
  an adapter lifecycle error: normal browser `pointerup -> lostpointercapture` canceled every staged
  click-click geometry interaction even though the direct Rust bridge authoring owner committed it.
  The viewport now retires exact pointer ownership synchronously before awaited release dispatch,
  genuine cancel/loss cancels once, and the bridge routes cancellation cleanup so construction
  paint clears and provisional Fillet/Profile Offset state restores without false Problems. Bridge
  `19/19`, demo-web `391/391`, frontend `19/19` and real release-WASM Playwright `8/8` pass.
  Immutable release-WASM snapshot `/tmp/geosolve-m88-react-uat.QkVU1k`, aggregate
  `e44bd8c22ccb67e542a8c73b58f62ed8ab236ab728b2d1bc2ae1aff1895ac167`, is byte-verified at
  `http://100.94.63.83:18088/` and remains preserved as F003 evidence.

  M88-F004 makes canvas chrome a projection of retained capability. The bridge owns exact Finish
  and history readiness; completion for computed tools requires preview/candidate identity; React
  owns only contextual labels, disabled presentation and a fixed feedback overlay. New-curve role
  and selected-curve role use distinct commands. Camera/first-Escape cancellation restores gesture
  state without silently deactivating its authoring collector, one-shot preselection returns to
  Select, and Code-only/dirty-draft/secondary-click routes no longer advertise or invoke unavailable
  semantic work. Bridge `23/23`, demo-web `395/395`, frontend `26/26` and release-WASM Playwright
  `10/10` pass. Snapshot `/tmp/geosolve-m88-react-uat.KGhA7s`, aggregate
  `700ebae4aec13ce20ab8786b63254e2c5b6239204c38bdc6e9d35f11c4159071`, is exact-served at the
  unchanged URL. The supervising user's milestone-level close decision accepts M88-U1 through
  M88-U10 without claiming a separately logged row replay. The obsolete Rust-DOM browser host,
  global installed callback registries and DOM-only dependencies/tests are retired from source;
  React now reaches Rust only through an instance-scoped `WorkbenchHandle`/`WorkbenchBridge`.
  The accepted F004 service remains live and no public deployment or service retirement is
  inferred. `docs/M88_AUDIT.md`,
  `docs/M88_GOALS.md`, `docs/M88_IMPLEMENTATION.md`, `docs/M88_UAT.md` and
  `docs/M88_HANDOVER.md` own the closed contract.
...
- **Completed target:** M78 implements ADR 0036's exact nine-family/25-variant geometry-
  authoring catalog, semantic draft stages and atomic recipe plans. Geometry kind remains a coarse
...
- **Completed target:** M79 hardens the existing M70/M71 inference lifecycle without adding
  geometry or solver semantics. `geosolve-constraint-editor` owns an immutable bounded cohort for
  one exact stationary `DraftInferenceFrame`, deterministic full-list cycling, stale preference authority and
  ...
  authority and immutable served-byte verification. Human UAT is accepted. Documentation-only
  approval descendant `2560ca5` passes Pages run `32116835502`, artifact `9317131695`, deployment
...
- **Completed target:** M80 implements amended ADR 0037's native, topology-preserving Profile Offset. One
...
- **Latest completed cleanup target:** M81 preserves the complete M80 product while making private
...
`e01d642438ae8337e9abe1ddeadb7b176375ae40f8b411785edae717e30b5d54`, was served by retained PID
`276377` at `http://100.94.63.83:8080/` after exact local, temporary and retained verification.
The supervising user accepted M83 on 2026-08-25 without claiming a separate row-by-row replay.
Approval descendant `2006c86`, tree `c4a59d2`, passes Pages run `32817232564`; downloaded artifact
...
PID `276377` is retired, its endpoint refuses connections, the immutable snapshot remains and M83
is closed.
...
- M76: completed the production-quality annotation cut. `geosolve-constraint-editor` owns exact
  ...
  closure and explicitly waived a separate post-refinement replay. M76 is complete.
- M77: implemented selected-curve control cages, exact trim/size/rational projections, immutable
  ...
  accepted baseline.
- M78: complete under ADR 0036. The approved target adds an exact nine-family/25-variant
  ...
  Approval descendant `a6d504e` passes Pages run `32096209036`, artifact `9310104202`, deployment
...
- M79: complete. Exact stationary candidate cohorts, headless wraparound, browser context retirement
  ...
  Tailscale bytes are verified and explicitly accepted. Approval descendant `2560ca5` passes Pages
...
- M81: complete behavior-preserving architecture consolidation. Private core solver, sketch query/
...
- M82: closed as a deferred, non-product computed all-family Offset exploration. The complete
  unaccepted architecture remains on `archive/m82-certified-computed-offset-2026-08-21` at
...
- M83: complete and closed 2026-08-25. ADR 0040's projectional graph, F001-F010 interaction
  repairs and post-F007 architecture hardening are implemented over the unchanged M81 solver/
...
being absent after reboot and is not a product defect.

Immutable snapshot `/tmp/geosolve-m89-f005-uat.hzNuDxF0`, manifest
`/tmp/geosolve-m89-f005-uat.hzNuDxF0.sha256` and aggregate
...
case passes `1/1` and the normal frozen product passes `15/15` on both endpoints. Unit
`geosolve-m89-react-uat-current.service`, PID `1007459`, invocation
`a3fea6dddb59438795e52c6a8fab136f`, was recorded serving only that snapshot at
`http://100.94.63.83:18089/`; automated evidence accepts no human row.
...
`/tmp/geosolve-m90-uat.vuI7sBKt`, with manifest `/tmp/geosolve-m90-uat.vuI7sBKt.sha256` and aggregate
...
`767e65048ece4573834acf8f580f87bc`, used the frozen snapshot as its working directory and served it
at `http://100.94.63.83:18090/`. M90-F001 withdraws those bytes from continuing UAT while preserving
all hashes and identities above as exact reproduction evidence. The unit is inactive/dead with no
PID; the endpoint is now reused only by the separately identified replacement below. At that
...
`/tmp/geosolve-m90-uat.xk0AGnz0`, manifest `/tmp/geosolve-m90-uat.xk0AGnz0.sha256`, aggregate
...
`geosolve-m90-f001-replacement-uat-18090.service` historically served the snapshot with PID
`3332035`, invocation `258e6d4da4b14661bd6d8e44856c64a5`, at
`http://100.94.63.83:18090/` on `100.94.63.83`; M90-F002 retired the unit.
...
raw IndexedDB authority and froze `/tmp/geosolve-m90-uat.TN2NP9eF`; M90-F003 now withdraws those
...
verified staging and live bytes. Snapshot `/tmp/geosolve-m90-uat.yIPVNICT`, manifest
`/tmp/geosolve-m90-uat.yIPVNICT.sha256`, aggregate
`d31e311c4e0e69974690d299819df6a33b7ae13b7eff5d037406e602c033f33a` and historical Tailscale-only
...
The current immutable optimized snapshot `/tmp/geosolve-m90-uat.EtWyWQlt`, manifest
...
release-WASM browser bundle passes `4/4` against each endpoint. Tailscale-only unit
...
and service remain Tailscale-only closing publication authority; no GitHub Pages deployment or public
...
its Tailscale-only service. The supervising user's blanket/composite approval accepts M91-U1 through
